...
# Source: https://github.com/RandomSearch18/POOP/blob/master/Crop%20management/menu.py
...
class Page(MenuItem):
    ...
    def execute(self, error_handling: Literal["restart", "return"] = "restart"):
        try:
            ...
            if error_handling == "restart":
                ...
                self.before_backward_navigation()
                return self.execute(error_handling)
            if error_handling == "return":
                self.before_backward_navigation()
                return
...
class Menu:
    ...
    def show(
        ...
        if self.title:
            print(color(self.title, Style.BRIGHT))

        if use_extra_linebreaks:
            # Padding between the title and the options
            print()
        ...
        try:
            selected_option.execute(error_handling=error_handling)
        except KeyboardInterrupt:
